 ...
 * 
 * // Accessing the value of a key of a known type, already converted to a
 * specific
 * // type (it simple uses casting!). It throws an exception if the type does
 * not
 ...
 * }
 * 
 * // check if two JSON structure are semantically equal: have same keys with
 * same
 ...
 * String jsonString =
 * "{ \"a\": 1234, \"b\": 2e-10, \"c\": \"Hola!\", \"d\": [1,2,3], \"e\": { \"k\" : 123,  \"h\" : \"Helloooo!\", \"f\": 23.3e-10 }}"
 * ;
 * JSONObject joFromString = new JSONObject(jsonString);
 * 
 * // build a JSONObject from a file
 * JSONObject joFromFile1 = new JSONObject(
 * new JSONTokener(new FileInputStream(new
 * File("resources/other/json-example-1.json"))));
 * JSONObject joFromFile2 = new JSONObject(
 * new JSONTokener(new FileInputStream(new
 * File("resources/other/json-example-2.json"))));
 * JSONObject joFromFile3 = new JSONObject(
 * new JSONTokener(new FileInputStream(new
 * File("resources/other/json-example-3.json"))));
 * 
 ...
 * 
 * System.out
 * .println("Are joFromString and joFromFile1 simrilar? " +
 * checkSemanticEquality(joFromString, joFromFile1));
 * System.out
 * .println("Are joFromString and joFromFile2 simrilar? " +
 * checkSemanticEquality(joFromString, joFromFile2));
 * System.out
 * .println("Are joFromString and joFromFile3 simrilar? " +
